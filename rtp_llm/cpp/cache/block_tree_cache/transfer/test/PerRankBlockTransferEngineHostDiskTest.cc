...
TEST_F(PerRankBlockTransferEngineHostDiskTest, MultipleDescriptorsUseOneVectorCallPerDirection) {
    ...
    EXPECT_EQ(executor.execute(hosts, writes, groups), TransferStatus::OK);
    ...
    EXPECT_EQ(executor.execute(hosts, reads, groups), TransferStatus::OK);
    ...
    EXPECT_EQ(executor.execute(hosts, writes, groups), TransferStatus::DISK_IO_ERROR);
