...
TEST(BlockTransferDispatcherTest, SingleDescriptorBatchUsesAsyncPerRankEntry) {
    std::shared_ptr<ScriptedPerRankEngine> engine = std::make_shared<ScriptedPerRankEngine>();
    BlockTransferDispatcher                dispatcher(engine);

    const auto context = dispatcher.executePerRank({TransferDescriptor::hostToDisk(0, 1, 1)});
    ASSERT_NE(context, nullptr);
    context->waitDone();
    EXPECT_TRUE(context->success());
    EXPECT_EQ(engine->submitCount(), 1u);
    EXPECT_EQ(engine->lastBatchSize(), 1u);
}
