...
TEST_F(PerRankBlockTransferEngineTest, SingleDescriptorBatchCompletesAsynchronously) {
...
void writeDiskPayload(const std::shared_ptr<BlockTreeDiskBlockPool>& disk_pool,
                      BlockIdxType                                    disk_block,
                      const std::vector<uint8_t>&                     payload) {
    HostStagingBlockPool staging_pool(1, disk_pool->strideBytes(), false);
    auto                 lease = staging_pool.malloc();
    ASSERT_TRUE(lease.has_value());
    const auto host = lease->blockBuffer(payload.size());
    std::memset(host.base, 0, host.capacity_bytes);
    std::memcpy(host.base, payload.data(), payload.size());
    EXPECT_EQ(disk_pool->write({disk_block}, {host.base}, disk_pool->strideBytes()), BlockIOStatus::OK);
}
...
TEST(PerRankBlockTransferEngineIntegrationTest, DiskToDeviceVectorSlicesByOneOfTwoStagingPools) {
    ...
    for (size_t index = 0; index < 5; ++index) {
        ...
        writeDiskPayload(disk_pool, disk_block, expected.back());
        fillDeviceLayer(device_pool, 0, device_block, {0x00, 0x00});
...
TEST(PerRankBlockTransferEngineIntegrationTest, DiskToDeviceOverlapsNextStageOneWithCurrentStageTwo) {
    ...
    for (size_t index = 0; index < 4; ++index) {
        ...
        writeDiskPayload(disk_pool, disk_block, readDeviceLayer(device_pool, 0, device_block));
        fillDeviceLayer(device_pool, 0, device_block, {0x00, 0x00});
        descriptors.push_back(makeDescriptor(Tier::DISK, Tier::DEVICE, {device_block}, NULL_BLOCK_IDX, disk_block, 0));
    }
...
TEST(PerRankBlockTransferEngineIntegrationTest, DiskDeviceStagingTransientExhaustionWaitsAndDeliversPayload) {
    ...
    writeDiskPayload(disk_pool, first_slot, expected_first);
    writeDiskPayload(disk_pool, second_slot, expected_second);
...
TEST_F(PerRankBlockTransferEngineStrategyTest, MultipleDescriptorsUseOnePlanPerDirectionAndDevice) {
    ...
    EXPECT_EQ(executor.execute(hosts, d2h_descriptors, groups), TransferStatus::OK);
    ...
    EXPECT_EQ(executor.execute(hosts, h2d_descriptors, groups), TransferStatus::OK);
...
TEST_F(PerRankBlockTransferEngineStrategyTest, BatchStrategyFailureFailsWholeExecutorCall) {
    ...
    EXPECT_EQ(executor.execute(hosts, descriptors, groups), TransferStatus::DEVICE_IO_ERROR);
    ASSERT_EQ(capture->plans.size(), 1u);
