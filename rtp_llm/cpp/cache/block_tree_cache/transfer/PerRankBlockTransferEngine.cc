...
std::shared_ptr<AsyncContext> PerRankBlockTransferEngine::submit(const TransferDescriptor& desc) {
    const GroupSet* group_set         = group_sets_[desc.group_set_id].get();
...
std::shared_ptr<AsyncContext> PerRankBlockTransferEngine::submit(const std::vector<TransferDescriptor>& descriptors) {
    ...
    for (const auto& descriptor : descriptors) {
        ...
        const auto      status    = validateBatchDescriptor(descriptor, group_set);
...
TransferStatus PerRankBlockTransferEngine::executeDirectBatch(const std::vector<TransferDescriptor>& descriptors,
    ...
    const auto path = classifyTransferPath(descriptors.front().source_tier, descriptors.front().target_tier);
    if (path == TransferPath::DEVICE_HOST) {
...
TransferStatus PerRankBlockTransferEngine::validateBatchDescriptor(const TransferDescriptor& desc,
    ...
    return path == TransferPath::DEVICE_HOST || path == TransferPath::DEVICE_DISK ?
               validateBatchDeviceBlocks(desc, *group_set) :
               TransferStatus::OK;
}

TransferStatus PerRankBlockTransferEngine::validateBatchDeviceBlocks(const TransferDescriptor& desc,
                                                                     const GroupSet&           group_set) const {
