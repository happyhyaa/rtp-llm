...
bool BlockTreeTaskPool::submitCompletion(std::function<void()> task) {
    ...
    completion_tasks_submitted_.fetch_add(1, std::memory_order_relaxed);
    const size_t depth = completion_queue_.size();
    size_t       peak  = completion_queue_peak_depth_.load(std::memory_order_relaxed);
    while (peak < depth
           && !completion_queue_peak_depth_.compare_exchange_weak(
